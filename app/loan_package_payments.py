...
class LoanPackagePayments(LoanPackage):
    '''
    Expanding for the LoanPackage Class which
    allows only single interest rate
    ...
    and their associated term period were provided in a list format.
    ...
    This computed installments will be paid for 1 year (12 payments).
    At the 13th payment (2nd year start mark),
    using the new interest rate from the list,
    the new installment figure will be recomputed
    for the remaining duration of the loan tenor (360-12) and the loan balance.
    If there is subsequent  rate, the installment will be recomputed
    again with the new loan balance and loan length.
    The above example could be changed based on the list
    provided such as using the same rate to pay for 1-2 years
    then change payment rate for following and so on.
    the concepts provided the different building components
    for the loan schedule catering to list interest rate and term periods.
    '''

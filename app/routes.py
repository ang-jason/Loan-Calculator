...
def index():
    ...
    if request.method == 'POST' and form.validate():
        ...
        # this statments store the values into the
        # session using flask transparent to user
...
def schedule():
    ...
    df_desc = "Loan Amount: " + str(T_LOANAMOUNT) + " Interest Rate(s): " \
        + str(T_RATE) + (" Terms: " + str(T_TERM) if T_TERM else "") + " Loan Tenor: " + str(T_TENOR)
    x = x.round(2)
    # print(x,type(x))
    ...
    return render_template("schedule2.html", data=x, name=df_text, desc=df_desc, summary=summary_dict, yearly_labels=yearly_labels, data_col_names=data_col_names,
        data_sbalance=data_sbalance, data_payment=data_payment, data_principal=data_principal, data_interest=data_interest)
